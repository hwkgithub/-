          ...
          <div :class="{on: loginWay}">
            <section class="login_message">
              <input type="tel"
                     ...
                     v-model="phone">
              <!-- 
                  这里是绑定一个类名，当rightPhone计算属性为true的时候
                  也就是手机号合法的时候 获取验证码几个字就变成了黑色 
              -->
  ...
  methods: {
    ...
    async getCode() {
      ...
      if (!this.computeTime) {
        ...
        //后台路由接口里的方法当code === 0的时候成功 code === 1的时候失败
        if (result.code === 1) {
          // 显示提示
    ...
    //获取一个新的图片验证码（触发重新发请求）
    //这里路径改变之后就会重新发请求 但是路径必须正确
    getCaptcha() {
      // 每次指定的src要不一样
      //?后面加参数time Date.now()显示系统当前时间
